...
The application reads assessment instrument files from a data/ subfolder located next to the compiled executable. If building from source, ensure each JSON file in data/ has its Visual Studio property set to Copy to Output Directory: Copy if newer. The program auto-discovers all valid JSON files in this folder at startup — no code changes are needed to add new instruments.
